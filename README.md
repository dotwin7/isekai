...
`managed-test`는 원본 Project가 아닌 일회용 복제 workspace에서 명령을 실행하고 authorization receipt에 exit code와 출력 digest를 결박합니다. 테스트가 상대 경로에 생성한 파일은 원본으로 돌아오지 않습니다. 별도 CI나 외부 runner가 제출한 Evidence도 계속 허용되며, 이 경우 `attestation.output_digest_verification`은 Core가 원문 output에서 digest를 계산했는지 또는 caller가 digest만 제출했는지를 보존합니다. 외부 실행 사실까지 보증해야 하는 환경에서는 보호된 CI나 서명된 원격 실행 receipt를 함께 사용해야 합니다.
