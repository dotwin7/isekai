...
ISEKAI는 중앙 실행 서비스가 아니라 대상 프로젝트 디렉터리에 배치되는 project-local Runtime이다. 설치기는 선택한 Runtime Skill, 프로젝트 로컬 Python Core, Catalog, 고정 Foundation과 lock을 함께 복사한다. 사용자는 대상 프로젝트에서 기존 Codex·Claude·Kiro를 시작하고 Skill을 명시적으로 활성화한다.
...
project-root/
├─ project.json
├─ .isekai/catalog/                  # release가 배포한 Catalog와 package
