...
Human Gate는 일회성 확인이 아니라 반복 가능한 검수 루프다. 사용자가 pending packet에 승인 대신 수정·보완·추가 요구사항을 답하면 Adapter는 이를 현재 `review_round`의 `rejected` Decision으로 기록하고 피드백을 반영한다. 수정과 검증을 마친 뒤 `status` 또는 `verify`를 다시 호출해 증가한 review round와 변경 내용을 담은 새 packet을 제시하고 명시적 승인을 다시 기다려야 한다. 추가 피드백이 오면 이 `reject → revise → verify → re-request` 흐름을 반복하며, 과거 승인을 수정 결과에 재사용하거나 피드백 반영 직후 완료로 종료해서는 안 된다. 피드백이 scope·stage plan·risk·필요 action을 바꾸면 Execution Envelope도 교체하고 새 Inception Decision을 받아야 한다.
