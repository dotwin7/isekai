...
Codex can pass an explicitly selected Skill and its remaining argument text as separate host context. Once Codex marks the `isekai` Skill as explicitly selected, a remaining argument such as `off` is the action even when the literal `$isekai` prefix is no longer model-visible. The Adapter must execute it rather than ask the user to repeat the command. Reading or pasting this Skill document is still not an invocation.
