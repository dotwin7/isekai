...
Kiro can load the explicitly selected workspace Skill separately from its remaining slash-command text. Once Kiro marks `isekai` as the active named Skill, a remaining argument such as `off` is the action even when the literal `/isekai` prefix is no longer model-visible. The Adapter executes it instead of asking the user to repeat the command. Reading or pasting `SKILL.md` is still not an invocation.
