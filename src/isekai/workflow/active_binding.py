...
from .active_binding_schema import (
    ACTIVE_BINDING_SCHEMA_VERSION as ACTIVE_BINDING_SCHEMA_VERSION,
...
def _load_binding(project_manifest: Path, project_id: str) -> dict[str, Any]:
    ...
    issues = binding_issues(value, project_id=project_id)
    if issues:
...
def _event(
    ...
    *,
    ...
    event["event_digest"] = binding_event_digest(event)
    return event
...
def _write_event(
    ...
    issues = binding_issues(candidate, project_id=str(binding.get("project_id")))
    if issues:
