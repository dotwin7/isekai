...
def test_tree_digest_ignores_directories_containing_only_transient_files(
    tmp_path: Path,
) -> None:
    source = tmp_path / "source"
    source.mkdir()
    (source / "tracked.py").write_text("VALUE = 1\n", encoding="utf-8")
    cache_only = source / "orphaned/__pycache__"
    cache_only.mkdir(parents=True)
    (cache_only / "module.pyc").write_bytes(b"cache")

    checkout = tmp_path / "checkout"
    checkout.mkdir()
    (checkout / "tracked.py").write_text("VALUE = 1\n", encoding="utf-8")

    assert tree_digest(source) == tree_digest(checkout)
...
def _bump_release(release: Path, version: str) -> None:
    ...
    for path in replacements:
        content = path.read_text(encoding="utf-8").replace("0.4.0", version).replace("0.3.1", version)
        path.write_text(content, encoding="utf-8")
    for path in (release / "foundation").rglob("*.json"):
        content = path.read_text(encoding="utf-8").replace("0.2.1", version)
        path.write_text(content, encoding="utf-8")
    entry_version = release / "catalog/ai-dlc/0.3.1"
    if entry_version.is_dir() and version != "0.3.1":
...
def test_project_install_is_pinned_idempotent_and_host_ready(tmp_path: Path) -> None:
    ...
    assert (
        project / ".isekai/catalog/ai-dlc/0.3.1/manifest.json"
    ).is_file()
    ...
    with pytest.raises(DistributionError, match="execution guard is not ready"):
        verify_adapter_handshake("codex", "0.4.0", "1.2.0", project)
    apply_execution_profile(project, "codex")
    handshake = verify_adapter_handshake("codex", "0.4.0", "1.2.0", project)
    ...
    with pytest.raises(DistributionError, match="adapter version does not match project lock"):
        verify_adapter_handshake("codex", "0.2.1", "1.2.0", project)
    ...
    assert json.loads(completed.stdout)["core_version"] == "0.4.0"
...
def test_doctor_fails_closed_after_installed_catalog_tampering(
    ...
    project = _project_with_foundation(tmp_path)
    _install(project)
    manifest = project / ".isekai/catalog/ai-dlc/0.3.1/manifest.json"
    manifest.write_text("{}\n", encoding="utf-8")
...
def test_update_preserves_foundation_and_rollback_restores_previous_release(
    ...
    manifest = json.loads((project / "project.json").read_text(encoding="utf-8"))
    manifest["version"] = "0.3.0-user-edited"
    (project / "project.json").write_text(
...
def test_malformed_install_lock_fails_closed_without_raw_cli_exception(
    ...
    health = doctor_install(project)
    exit_code = cli_main(
        ["update", "--check", "--ref", "v0.4.0", "--path", str(project)]
    )
...
def test_rollback_failure_restores_current_project_lock_and_adapters(
    ...
    def fail_previous_lock(
        ...
        if Path(relative) == Path("isekai.lock.json") and value.get("release") == "0.4.0":
