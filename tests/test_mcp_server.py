...
def test_mcp_artifact_write_persists_plan_through_core(tmp_path: Path) -> None:
    project = project_with_profile(tmp_path)
...
def test_mcp_server_rejects_unit_from_another_project(tmp_path: Path) -> None:
    first_root = tmp_path / "first"
    second_root = tmp_path / "second"
    first_root.mkdir()
    second_root.mkdir()
    first = project_with_profile(first_root)
